...
function buildSystemPrompt({ invokeLabel, invokeEntityType, contextDescription, contextBlock, session, userMemoryBlock, entityResolutionBlock, resolvedEntityObjects, userCurrency, userName, userLanguage, userTimezone, userHiddenSignals, lastShownEntities }) {
  ...
  const lines = [
    ...
    '',
    'PLAN-ITEM IDEAS — interactive picker is REQUIRED:',
    '- Whenever the user asks for plan-item ideas, suggestions, recommendations, or "what should I add/do/see/eat" for a destination, plan, or experience — OR you would otherwise list 2+ plan-item ideas in your message — you MUST emit a `suggest_plan_items` pending action and let the picker UI surface the options.',
    '- DO NOT list plan-item ideas as plain markdown bullets (e.g. "- Visit the medina\\n- See Sidi Abderrahman island"). Bulleted lists of ideas in the `message` field are a regression and bypass the interactive checklist the user expects.',
    '- The message field should be a brief one-line introduction to the picker (e.g. "Here are some popular ideas in Casablanca — pick the ones you want to add.") — never a duplicate list of the same items.',
    '- Required payload: `destination_id` (read it from session context — every plan and experience belongs to a destination). Optional: `experience_id` to scope to similar experiences, `exclude_items` to omit items already in the user\'s plan.',
    '- If `destination_id` is genuinely not available anywhere in the context, ask the user which destination they mean BEFORE listing ideas — never inline-bullet ideas in lieu of the picker.',
    '- Exception: when the user explicitly asks for a single specific recommendation ("what\'s the one thing I should do in X?"), a one-line answer in the message is acceptable — the picker rule applies to lists of ideas (2+).',
    '',
  ...
  lines.push(
    ...
    '  Returns suggestions ranked by frequency, then enriched server-side with Wikivoyage and TripAdvisor (when TRIPADVISOR_API_KEY is configured) — both external sources are queried on every call, so you do NOT need to fan out to them yourself for plan-item suggestions. The user can then pick which items to add.',
