...
**CRITICAL: Use Single Activity Tracker for All Activity Types**

⚠️ **DO NOT create separate activity trackers for different domains** (auth, payments, notifications, etc.)

**Why?**
- Maintains single source of truth for activity tracking
- Ensures consistent metadata capture and error handling
- Prevents code duplication and drift
- Easier to maintain and extend
- All activities queryable from one place

**Pattern:**
```javascript
// ✅ CORRECT: Extend existing activity-tracker.js
// utilities/activity-tracker.js
module.exports = {
  // Resource tracking
  trackCreate,
  trackUpdate,
  ...
  // Utilities (shared by all)
  extractMetadata,
  extractActor
};

// ❌ WRONG: Creating separate trackers
// utilities/auth-activity-tracker.js  // Don't do this
// utilities/payment-activity-tracker.js  // Don't do this
```

**Implementation Guidelines:**
1. **All tracking functions go in `utilities/activity-tracker.js`**
2. **Group functions by domain** with comments (Resource, Auth, Payment, etc.)
3. **Reuse utility functions** (extractMetadata, extractActor, etc.)
4. **All tracking must be non-blocking** (fire-and-forget with .catch())
5. **Consistent schema usage** for all Activity.create() calls

**Example Extension:**
```javascript
// Adding payment tracking to activity-tracker.js
async function trackPayment(options) {
  const { user, req, amount, currency } = options;
  ...
  Activity.create({
    ...
    backendLogger.error('Failed to track payment', {
      error: err.message,
      userId: user._id
    });
  });
